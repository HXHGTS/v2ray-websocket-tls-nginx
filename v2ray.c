...
Menu:UI();
    ...
    if (mode == 1) {
        ...
        scanf("%s", sni);
        system("echo \"------------------------------------------------------------------------------------\" > /usr/local/etc/v2ray/certificate.pem");
        system("echo \"请将pem或cer或pem格式的证书用记事本打开并将内容粘贴至此处. . .\" > /usr/local/etc/v2ray/certificate.pem");
        system("echo \"英文输入法下按i开始编辑，结束编辑先按ESC再按:wq回车以保存\" >> /usr/local/etc/v2ray/certificate.pem");
        system("echo \"-----------------以上为使用说明，编辑时请先删除所有文字再开始编辑！-------------------\" >> /usr/local/etc/v2ray/certificate.pem");
        system("vi /usr/local/etc/v2ray/certificate.pem");
        system("echo \"------------------------------------------------------------------------------------\" > /usr/local/etc/v2ray/private.pem");
        system("echo \"请将pem格式的私钥用记事本打开并将内容粘贴至此处. . .\" > /usr/local/etc/v2ray/private.pem");
        system("echo \"英文输入法下按i开始编辑，结束编辑先按ESC再按:wq回车以保存\" >> /usr/local/etc/v2ray/private.pem");
        system("echo \"-----------------以上为使用说明，编辑时请先删除所有文字再开始编辑！-------------------\" >> /usr/local/etc/v2ray/private.pem");
        system("vi /usr/local/etc/v2ray/private.pem");
...
int UI() {
    ...
    printf("----------------------v2ray安装工具-----------------------\n");
    printf("-----------------------------------------------------------\n");
    printf("安装前，请将证书与私钥分别命名为1.pem与2.pem，上传至服务器/root目录");
    printf("-----------------------------------------------------------\n");
    printf("----------------------当前Kernel版本-----------------------\n");
    system("uname -sr");
    printf("-----------------------------------------------------------\n");
